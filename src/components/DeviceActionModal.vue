...
const props = defineProps({
  ...
  loading: { type: Boolean, default: false }, // parent can pass to show spinner on confirm
})
const emit = defineEmits(['close', 'confirm']) // confirm(deviceId)
    ...
    <div class="bg-white rounded-xl shadow-xl w-[520px] max-w-[95vw]">
      ...
      <div class="p-4 space-y-3 text-sm">
        <div class="text-slate-600">
          ...
          <div v-if="mode === 'push'">Select a device to <b>push</b> this user into.</div>
      ...
      <div class="px-4 py-3 border-t flex items-center justify-end gap-2">
        ...
        >
          <i
            v-if="!loading"
            class="fas"
            :class="mode === 'push' ? 'fa-upload' : 'fa-user-slash'"
          ></i>
          <i v-else class="fas fa-circle-notch fa-spin"></i>
          <span class="ml-1">{{ mode === 'push' ? 'Confirm Push' : 'Confirm Remove' }}</span>
        </button>
      </div>
    </div>
  </div>
