...
<template>
  <div class="border text-gray-800 py-4 shadow-sm mb-4 px-4 rounded-md hover:bg-blue-50">
    <div>
      <div class="text-lg font-semibold mb-2">
        <span>{{ serial }}. {{ detail.title }}</span>
      </div>
      <hr class="mb-4 -mx-4" />
      <div class="line-clamp-5 mb-5">
        <div class="text-gray-500 mr-2 text-xs uppercase font-semibold">Description</div>
    ...
    <div class="flex items-end">
      <div>
        ...
        <div>
          <span class="text-gray-500 mr-2 text-xs uppercase font-semibold"
            >Better To Complete:</span
          >
          <span>{{ detail.better_to_complete_on }}</span>
        </div>
      </div>
