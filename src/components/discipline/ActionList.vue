...
const itemHours = (item) => item?.hours || item?.pay_hours || item?.paycut_hours || item?.pay_hours_text
</script>
...
<template>
  <div class="space-y-1">
    <div
      ...
      :key="`auto-${item?.id || itemLabel(item)}`"
      class="list-item list-item--locked"
    >
      <div class="flex items-center gap-2">
        <i class="far fa-lock text-[11px]"></i>
        <span class="truncate">{{ item?.label || 'Paycut applied' }}</span>
      </div>
      <span v-if="itemHours(item)" class="badge">{{ itemHours(item) }}h</span>
    </div>
...
.badge { @apply rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-semibold text-amber-700; }
