...
const unreadCount = computed(() => policies.value.filter((p) => !p.user_feedback).length)
</script>
  ...
  <section class="relative overflow-hidden px-4 py-6 md:px-6">
    <div class="relative mx-auto max-w-5xl">
      <div class="mb-6 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p class="text-sm md:text-base text-slate-600">
            Keep up with company rules, HR updates, and compliance notes.
          </p>
        </div>
        ...
        <div class="flex items-center gap-3">
          <div class="rounded-xl bg-white/80 px-4 py-2 shadow ring-1 ring-slate-200">
            ...
            <p class="text-lg font-semibold text-slate-900">{{ policies.length }}</p>
          </div>
      ...
      <ul v-else class="grid gap-3 md:grid-cols-2">
        <li v-for="notification in policies" :key="notification.id">
          <RouterLink
            :to="{ name: 'PolicyDetails', params: { id: notification.id } }"
        ...
        <!-- No Policies -->
        <li v-if="policies.length === 0" class="md:col-span-2">
          <div class="rounded-2xl bg-white/80 p-6 text-center ring-1 ring-slate-200">
            <p class="text-slate-600">No policies found. You are all caught up.</p>
          </div>
        </li>
      </ul>
    </div>
