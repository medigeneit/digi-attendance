...
onMounted(async () => {
  loading.value = true
  await bugStore.fetchBug(bugId)
  await userStore.fetchUsers() // all available users
  selectedUsers.value = bugStore?.bug?.users
  loading.value = false
})
...
const submit = async () => {
  ...
  try {
    await bugStore.assignUsers(bugId, user_ids.value)
  ...
  <div class="container mx-auto p-6">
    <div class="max-w-xl mx-auto bg-white shadow-lg rounded-lg p-6">
      <h2 class="text-2xl font-semibold text-gray-800 mb-4">
        Assign Users to Bug: "{{ bugStore?.bug?.title }}"
      </h2>
