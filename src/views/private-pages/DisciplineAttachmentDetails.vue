...
const fileUrl = computed(
  () => attachment.value?.file || attachment.value?.file_url || attachment.value?.url || ''
)

const isAcknowledged = computed(
  () => !!attachment.value?.acknowledged_at || !!attachment.value?.acknowledged_by
)
...
const fetchAttachment = async () => {
  try {
    ...
    const data = await reportStore.showAttachment(attachmentId)
    attachment.value = data
  } catch (err) {
    error.value = err?.response?.data?.message || 'Failed to load attachment details'
...
const acknowledgeNow = async () => {
  const attachmentId = attachment.value?.id || route.params.id
  if (!attachmentId || isAcknowledged.value) return
  try {
    isLoading.value = true
    await reportStore.acknowledgeAttachment(attachmentId)
    const refreshed = await reportStore.showAttachment(attachmentId)
    attachment.value = refreshed
    toast.success('Attachment acknowledged')
  } catch (err) {
    error.value = err?.response?.data?.message || 'Failed to acknowledge'
    toast.error(error.value)
  } finally {
    isLoading.value = false
  }
...
watch(
  ...
  (data) => {
    useHead({
      title: data?.title || 'Discipline Attachment',
      meta: [
        { property: 'og:title', content: data?.title || 'Discipline Attachment' },
        { property: 'og:image', content: fileUrl.value || noitceImage },
        { property: 'og:description', content: data?.note || data?.description || '' },
      ],
    })
  },
  { immediate: true }
...
<template>
  <div class="my-container py-6 print:px-0">
    <!-- Sticky action bar -->
    <div
      v-show="showStickyBar && attachment"
      ...
      <div class="mx-auto max-w-6xl px-3 py-2 flex items-center justify-between gap-2">
        <div class="flex items-center gap-2 min-w-0">
          ...
          <div class="truncate font-medium text-sm text-gray-800">
            {{ attachment?.title?.replace(/<[^>]*>?/gm, '') || 'Discipline Attachment' }}
          </div>
        </div>
        <div class="flex items-center gap-2">
          <button class="btn" @click="copyLink"><i class="far fa-link mr-1"></i>Copy</button>
          <button v-if="fileUrl" class="btn" @click="openInNewTab(fileUrl)">
            <i class="far fa-arrow-up-right-from-square mr-1"></i>Open
          </button>
          <button v-if="fileUrl" class="btn" @click="downloadDirect(fileUrl, getFilename(fileUrl))">
    ...
    <div class="mx-auto max-w-6xl">
      ...
      <div class="rounded-2xl overflow-hidden bg-white shadow-sm ring-1 ring-gray-100">
        ...
        <div class="px-6 py-5 flex items-start justify-between gap-4">
          <div class="min-w-0">
            <div class="text-[11px] uppercase tracking-wider text-gray-500">Discipline Attachment</div>
            <h1
              class="mt-1 text-xl md:text-2xl font-semibold text-gray-900 leading-snug"
              v-html="attachment?.title || 'Attachment Details'"
            ></h1>
            <div class="mt-2 flex flex-wrap items-center gap-2 text-xs text-gray-500">
              <span>
                Created:
                <span class="font-medium text-gray-700">{{ formatDate(attachment?.created_at) }}</span>
              </span>
              <span v-if="attachment?.month_start">
                - Month:
                <span class="font-medium text-gray-700">{{ attachment?.month_start }}</span>
              </span>
              <span v-if="attachment?.user_name || attachment?.employee_name">
                - Employee:
                <span class="font-medium text-gray-700">
                  {{ attachment?.user_name || attachment?.employee_name }}
                </span>
              </span>
            </div>
          </div>
          ...
          <div class="shrink-0 flex flex-col items-end gap-2">
            <span v-if="!isAcknowledged" class="badge-amber" title="Pending acknowledgment">
              <span class="h-2 w-2 rounded-full bg-amber-500"></span> Pending Acknowledgment
            </span>
            <span v-else class="badge-emerald" title="Acknowledged">
              <i class="far fa-check"></i> Acknowledged
            </span>
      ...
      <div class="mt-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-8 space-y-6">
          ...
          <div
            ...
            v-if="attachment?.description || attachment?.note || attachment?.body_html"
          >
            <div class="px-6 py-5">
              ...
              <div v-else>
                <div
                  v-if="attachment?.description || attachment?.note"
                  class="prose max-w-none prose-img:my-0 text-gray-800"
                  v-html="attachment?.description || attachment?.note"
                ></div>
                <div
                  v-else-if="attachment?.body_html"
                  class="prose max-w-none prose-img:my-0 text-gray-800"
                  v-html="attachment?.body_html"
                ></div>
          ...
          <!-- Attachment -->
          <div v-if="fileUrl" class="rounded-2xl bg-white shadow-sm ring-1 ring-gray-100 overflow-hidden">
            <div class="px-6 py-4 md:flex items-center justify-between">
              ...
              <button
                class="text-sm underline text-indigo-600 truncate hover:text-indigo-700"
                @click="downloadDirect(fileUrl, getFilename(fileUrl))"
              >
            ...
            <div class="px-6 pb-6">
              <!-- IMAGE -->
              <div v-if="isImage(fileUrl)" class="relative rounded-xl overflow-hidden border bg-gray-50">
                ...
                <div v-if="imgError" class="p-6 flex items-center justify-between">
                  <div class="flex items-center gap-3 text-gray-700">
                    ...
                    <div>
                      ...
                      <p class="text-sm text-gray-500 break-all">{{ getFilename(fileUrl) }}</p>
                    </div>
                  </div>
                  <div class="flex items-center gap-2">
                    <button class="btn" @click="openInNewTab(fileUrl)">Open</button>
                    <button class="btn-primary" @click="downloadDirect(fileUrl, getFilename(fileUrl))">Download</button>
                  </div>
                </div>

                <div v-if="!imgError" class="absolute top-2 right-2 flex items-center gap-2">
                  <button class="btn-4 py-1" title="Download" @click="downloadDirect(fileUrl, getFilename(fileUrl))">
                    <i class="far fa-download"></i>
                  </button>
                </div>

                <div class="flex items-center justify-between px-4 py-2 border-t bg-white">
                  <div class="truncate text-sm text-gray-700" :title="getFilename(fileUrl)">
                    <span class="font-medium">Image:</span> {{ getFilename(fileUrl) }}
                  </div>
                  <div class="flex items-center gap-2">
                    <button class="btn" @click="openInNewTab(fileUrl)">Open</button>
                    <button class="btn-primary" @click="downloadDirect(fileUrl, getFilename(fileUrl))">Download</button>
              ...
              <!-- PDF -->
              <div v-else-if="isPDF(fileUrl)" class="rounded-xl overflow-hidden border bg-white relative">
                <iframe :src="pdfViewerUrl" class="w-full h-[80vh] bg-white" title="Attachment PDF"></iframe>
                <div class="absolute top-2 right-2 flex items-center gap-2">
                  <button class="btn-4 py-1" title="Open in new tab" @click="openInNewTab(fileUrl)">
                ...
                <div class="flex items-center justify-between px-4 py-2 border-t bg-white">
                  <div class="truncate text-sm text-gray-700">
                    <span class="font-medium">PDF:</span> {{ getFilename(fileUrl) }}
                  </div>
                  <div class="flex items-center gap-2">
                    <button class="btn" @click="openInNewTab(fileUrl)">Open</button>
                    <button class="btn-primary" @click="downloadDirect(fileUrl, getFilename(fileUrl))">Download</button>
              ...
              <div v-else class="rounded-xl bg-gray-50 p-4 border flex items-center justify-between">
                <div class="text-gray-700 truncate" :title="getFilename(fileUrl)">
                  <span class="font-medium">File:</span> {{ getFilename(fileUrl) }}
                </div>
                <div class="flex items-center gap-2">
                  <button class="btn" @click="openInNewTab(fileUrl)">Open</button>
                  <button class="btn-primary" @click="downloadDirect(fileUrl, getFilename(fileUrl))">Download</button>
        ...
        <aside class="lg:col-span-4 space-y-6">
          <div class="rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
            <div class="px-6 py-5 space-y-3">
              ...
              <div class="grid grid-cols-2 gap-3 text-sm">
                <div class="rounded-lg bg-gray-50 p-3">
                  ...
                  <div class="font-medium text-gray-800">{{ formatDate(attachment?.created_at) }}</div>
                </div>
                <div class="rounded-lg bg-gray-50 p-3">
                  <div class="text-[11px] text-gray-500">Doc Type</div>
                  <div class="font-medium text-gray-800">{{ attachment?.doc_type || 'N/A' }}</div>
                </div>
                <div class="rounded-lg bg-gray-50 p-3 col-span-2">
                  <div class="text-[11px] text-gray-500">Mode</div>
                  <div class="font-medium text-gray-800">{{ attachment?.mode || 'N/A' }}</div>
                </div>
              </div>

              <div class="pt-3 flex flex-wrap gap-2">
                <span class="chip">Discipline</span>
                <span v-if="!isAcknowledged" class="chip chip-amber">Pending</span>
                <span v-else class="chip chip-emerald">Acknowledged</span>
              </div>
            </div>
            ...
            <div class="px-6 pb-5 flex flex-col gap-2">
              ...
              <button v-if="fileUrl" class="btn w-full" @click="openInNewTab(fileUrl)"><i class="far fa-arrow-up-right-from-square mr-2"></i>Open attachment</button>
              <button v-if="fileUrl" class="btn-primary w-full" @click="downloadDirect(fileUrl, getFilename(fileUrl))"><i class="far fa-download mr-2"></i>Download</button>
            </div>
          </div>
            ...
            <div class="px-6 py-5">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                  ...
                  <h3 class="text-base md:text-lg font-semibold text-gray-900">Acknowledgment</h3>
                </div>
              </div>

              <div class="mt-4 space-y-3">
                <div v-if="isAcknowledged" class="rounded-xl bg-emerald-50 border border-emerald-200 px-4 py-3 text-emerald-800">
                  <div class="flex items-start gap-3">
                    <i class="far fa-check-circle mt-0.5"></i>
                    <div>
                      <div class="text-sm font-semibold">Acknowledged</div>
                      <div class="text-sm mt-0.5">
                        {{ formatDate(attachment?.acknowledged_at) }}
                        <span v-if="attachment?.acknowledged_by">- {{ attachment?.acknowledged_by }}</span>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      >
        <div class="flex items-center md:mt-14 justify-end p-3 text-white">
          <div class="flex items-center gap-2">
            ...
            <button class="btn-2" title="Download" @click="downloadDirect(fileUrl, getFilename(fileUrl))">
              <i class="far fa-download"></i>
            </button>
            <button class="btn-4 py-1 text-red-500" title="Close" @click="closeLightbox">x</button>
          </div>
        </div>
        ...
        >
          ...
          >
            <img
              :src="fileUrl"
              :alt="getFilename(fileUrl) || 'Attachment Image'"
