...
const fetchData = () =>
  exchangeStore.fetchExchanges({ payload: { type, date: periodMonth.value } })

watch(periodMonth, (v) => { if (v) fetchData() }, { immediate: true })
...
async function deleteApplication(id) {
  if (!id || !confirm('Delete this offday exchange request?')) return
  try {
    await exchangeStore.deleteExchange(id)
    await fetchData()
  } catch {
    alert(exchangeStore.error || 'Failed to delete.')
  }
}
</script>
    ...
    <div class="px-4 py-3">
      ...
      <div v-else class="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        ...
        <div v-if="list.length" class="overflow-x-auto">
          <table class="min-w-[580px] w-full text-sm">
            ...
            <thead>
              <tr class="bg-gray-50 border-b border-gray-200 text-[10px] uppercase tracking-wider text-gray-500">
                ...
                <th class="px-3 py-2.5 text-left min-w-[110px]">Applied</th>
                <th class="px-3 py-2.5 text-center min-w-[110px]">Holiday / Weekend</th>
                <th class="px-3 py-2.5 text-center min-w-[110px]">Work On</th>
            ...
            <tbody class="divide-y divide-gray-100">
              ...
              >
                ...
                <td class="px-3 py-2.5">
                  ...
                  <div v-if="ex?.reason" class="mt-0.5 text-[10px] text-gray-400 truncate max-w-[130px]" :title="ex.reason">
                    {{ ex.reason }}
                  </div>
                </td>

                <!-- Holiday/weekend (current_date) -->
                <td class="px-3 py-2.5 text-center">
                  <span class="inline-flex items-center rounded-md bg-orange-50 px-2 py-0.5 text-[10px] font-semibold text-orange-700 ring-1 ring-inset ring-orange-100">
                    {{ formatDate(ex?.current_date) }}
                  </span>
                </td>

                <!-- Work on (exchange_date) -->
                <td class="px-3 py-2.5 text-center">
                ...
                <td class="px-3 py-2.5">
                  <div class="flex items-center justify-center gap-1">
                    <RouterLink
                      :to="{ name: 'ExchangeOffdayShow', params: { id: ex?.id } }"
        ...
        <div v-else class="flex flex-col items-center justify-center py-16 text-center">
          <div class="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center mb-3">
            <i class="fas fa-umbrella-beach text-xl text-gray-300"></i>
          </div>
          <p class="font-semibold text-gray-700 text-sm">No offday exchange requests</p>
          <p class="mt-1 text-xs text-gray-400">for {{ selectedMonthLabel }}</p>
          <RouterLink :to="{ name: 'OffdayExchangeAdd' }" class="btn-2 mt-4 py-1.5 text-xs">
