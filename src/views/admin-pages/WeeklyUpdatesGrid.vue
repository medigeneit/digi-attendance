...
if (route.query.anchor_date) {
  params.value.anchor_date = String(route.query.anchor_date)
}
if (route.query.past_days != null) {
  params.value.past_days = clampDays(route.query.past_days, params.value.past_days)
}
if (route.query.future_days != null) {
  params.value.future_days = clampDays(route.query.future_days, params.value.future_days)
}
  ...
  router
    .replace({
      query: {
        ...
        past_days:
          params.value.past_days != null ? String(params.value.past_days) : undefined,
        future_days:
          params.value.future_days != null ? String(params.value.future_days) : undefined,
      },
    })
    .catch(() => {})
}
...
const shiftRange = (direction) => {
  ...
  params.value.anchor_date = nextValue
  store.fetchWeekly({
    anchor_date: nextValue,
    past_days: past,
    future_days: future,
    company_id: filters.value.company_id || '',
    department_id: filters.value.department_id || 'all',
...
/**
 * ✅ LAST item selector:
 * - iterate dates from end
 * - pick last date that has any items
 * - return last item of that day
 */
...
const openSmsForRow = (row) => {
  ...
  if (!last) {
    showToast('info', 'No application found in this range.')
    return
  }
  ui.value.smsTarget = {
    item: last,
...
const sendForItem = async (item, message = '') => {
  if (!item?.kind || !item?.ref_id) return
  ...
  try {
    ...
    const msg = e?.response?.data?.message || 'Failed to send SMS.'
    showToast('error', msg)
    return false
  } finally {
...
const sendSms = async () => {
  const target = ui.value.smsTarget
  ...
  const sent = await sendForItem(item, message)
  if (sent) closeSmsModal()
}
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
      <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div class="flex flex-wrap items-center gap-2">
          ...
          <div class="rounded-xl border border-slate-200 bg-slate-50/70 px-2 py-1">
            <div class="flex items-center gap-2">
              <label class="flex items-center gap-2 text-xs font-semibold text-slate-600">
                Past
                <input v-model.number="params.past_days" type="number" min="0" max="60"
                  class="w-14 rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs focus:border-slate-400 focus:outline-none" />
              </label>

              <label class="flex items-center gap-2 text-xs font-semibold text-slate-600">
                Future
                <input v-model.number="params.future_days" type="number" min="0" max="60"
                  class="w-14 rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs focus:border-slate-400 focus:outline-none" />
              </label>
            </div>
          </div>
          ...
          <label class="flex items-center gap-2 text-xs font-semibold text-slate-600">
            <input v-model="params.anchor_date" type="date"
      ...
      <div class="overflow-x-auto">
        <table class="min-w-max w-full text-sm">
          <thead class="bg-slate-50 text-slate-600">
            <tr>
              <th class="sticky left-0 top-0 z-30 border-r border-slate-200 bg-slate-50 px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide">
                Employee
              </th>

              <th v-for="date in dates" :key="date" class="sticky top-0 z-20 min-w-[110px] bg-slate-50 px-3 py-3 text-left">
              ...
              <th class="sticky top-0 z-20 bg-slate-50 px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide">
                SMS
              </th>
            </tr>
          </thead>
          ...
          <tbody>
            <tr v-for="row in rows" :key="row.user.id" class="border-t border-slate-300 hover:bg-slate-50/60">
              <td class="sticky left-0 z-20 border-r border-slate-200 bg-white px-4 py-3">
                <div class="min-w-0">
              ...
              <td class="px-4 py-3 text-right">
                ...
                <!-- <div v-if="getLastItem(row)" class="mt-1 text-[11px] text-slate-500">
                  Last: {{ getLastItem(row)?.code }} #{{ getLastItem(row)?.ref_id }}
                </div> -->
              </td>
            </tr>

            <tr v-if="!rows.length" class="border-t border-slate-100">
              <td class="bg-white px-4 py-8 text-slate-500" :colspan="dates.length + 2">
                No updates found.
              </td>
            </tr>
          </tbody>
        ...
        <div class="mt-3 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
          ...
          <div class="mt-1 flex flex-wrap items-center gap-2">
            ...
            <span class="text-[11px] text-slate-500">
              #{{ ui.smsTarget?.item?.ref_id || 'N/A' }}
            </span>
            <span class="text-[11px] text-slate-500">
              {{ ui.smsTarget?.item?.status || 'Unknown' }}
            </span>
          </div>
