...
export const useShortLeaveStore = defineStore('shortLeave', () => {
  ...
  const handoverAccept = async (payload) => {
    ...
    try {
      ...
      error.value =
        err.response?.data?.message || `Failed to accept handover for short leave (ID: ${id})`
  ...
  const inChargeAccept = async (payload) => {
    ...
    try {
      ...
      error.value =
        ...
        `Failed to accept In-Charge for short leave (ID: ${payload.id})`
  ...
  const recommendByAccept = async (payload) => {
    ...
    try {
      const response = await apiClient.post(
        `/short-leaves/${payload.id}/recommend-by-accept`,
        payload,
      )
      const index = shortLeaves.value.findIndex((leave) => leave.id === payload.id)
      if (index !== -1) {
      ...
      error.value =
        err.response?.data?.message ||
        `Failed to accept recommend by for short leave (ID: ${payload.id})`
  ...
  const approvedByAccept = async (payload) => {
    ...
    try {
      const response = await apiClient.post(
        `/short-leaves/${payload?.id}/approved-by-accept`,
        payload,
      )
      const index = shortLeaves.value.findIndex((leave) => leave.id === payload.id)
      if (index !== -1) {
      ...
      error.value =
        err.response?.data?.message ||
        `Failed to accept approval for short leave (ID: ${payload.id})`
